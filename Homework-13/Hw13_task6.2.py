...
def validate(student: dict) -> None:
    ...
    if not (1 <= student['average_score'] <= 100):
        is_validate = False
    if not is_upper(student['last_name']):
        is_validate = False
    if not is_upper(student['first_name']):
        is_validate = False
